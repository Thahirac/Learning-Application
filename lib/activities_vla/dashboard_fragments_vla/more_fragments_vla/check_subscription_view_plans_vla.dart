...
class CheckSubscriptionViewPlansVLA extends StatefulWidget {
  var thisSetNewScreenFunc;

  CheckSubscriptionViewPlansVLA(this.thisSetNewScreenFunc, {Key? key})
      : super(key: key);

  @override
  State<CheckSubscriptionViewPlansVLA> createState() =>
      _CheckSubscriptionViewPlansVLAState();
}

class _CheckSubscriptionViewPlansVLAState
    extends State<CheckSubscriptionViewPlansVLA> {
  ...
  Future<void> getTutorialVideoAPI() async {
    var url = Uri.parse(
        ConstantValuesVLA.baseURLConstant + ConstantValuesVLA.plansConstant);
    var viewPlansJson = {
        ...
        plansWidgetList.add(
          SlideTransitionCustomVidwath(
            ...
            GestureDetector(
              onTap: () {
                selectedPlansModelVLA =
                    PlansModelVLA.fromJson(tutorialVideoListTemp[i]);
                ...
                setState(() {
                  ...
                  totalAmount;
                  //print('********************TOTAL AMOUNT **************************$totalAmountStringForTextview');
  ...
  void checkPromoCodeAPI() {
    http.post(
            Uri.parse(ConstantValuesVLA.baseURLConstant +
                ConstantValuesVLA.promocodeConstant),
            body: jsonEncode({
              "subject_id": "0",
  ...
  void callApiSuccessDataCapture(String orderId) async{
    ...
    await _paymentGatewayApi.updateSuccessSubscription(totalAmount!,subjectId,subjectName, selectedPlansModelVLA.class_validity, orderId).then((value) async{

       //SubscriptionDao _subscriptionDao = SubscriptionDao.fromJson(value);
       // SharedPreferences preferences = await SharedPreferences.getInstance();
       // preferences.setString('Class_subscribe', _subscriptionDao.classSubscribe!);
